...
def compute_score_stats(domain, instance, g_obs_keys, ga_keys, ac_dict_fn, cfg, mode, rddl_model, cfg_env, s_weight,  n_restarts, n_episodes=10, time_budget = 1200, time_ps=1):
    start = time.time()
    ...
    agg_reward, ep_time, prev_ac_seq, agent_key = eval_episode(g_obs_keys, ga_keys, env, agent, agent_key, prev_ac_seq, lrs_to_scan, ac_dict_fn, time_ps)
...
def eval_episode(g_obs_keys, ga_keys, env, agent, agent_key, prev_ac_seq, lrs_to_scan, ac_dict_fn, time_ps):
    budget = env.horizon * time_ps
    start = time.time()
    state = env.reset()
    done = False
    agg_reward = 0
